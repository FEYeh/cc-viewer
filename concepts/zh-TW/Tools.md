...
工具呼叫是 Claude Code 請求日誌中最核心的可觀測資料之一。每個 MainAgent 請求的 `tools` 陣列定義了模型可用的工具集，而回應中的 `tool_use` / `tool_result` content block 記錄了實際的工具呼叫與回傳。cc-viewer 透過解析這些資料，在 Chat Mode 中還原完整的工具呼叫鏈路。

工具定義的變化（如 MCP server 連線/斷開導致工具增減）會觸發快取重建，詳見 [CacheRebuild](CacheRebuild.md)。
