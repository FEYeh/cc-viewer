...
ツール呼び出しは Claude Code リクエストログにおける最も重要な観測可能データの一つです。各 MainAgent リクエストの `tools` 配列はモデルが利用可能なツールセットを定義し、レスポンス内の `tool_use` / `tool_result` content block は実際のツール呼び出しと返却を記録します。cc-viewer はこれらのデータを解析し、Chat Mode で完全なツール呼び出しチェーンを再現します。

ツール定義の変化（MCP server の接続/切断によるツールの増減など）はキャッシュ再構築をトリガーします。詳細は [CacheRebuild](CacheRebuild.md) を参照してください。
