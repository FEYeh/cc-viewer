...
Task 呼び出しは SubAgent リクエストチェーンを生成し、リクエストリストで MainAgent とは独立したサブリクエストシーケンスとして確認できます。SubAgent リクエストは通常、簡潔な system prompt と少ないツール定義を持ち、MainAgent と明確な対比を形成します。
