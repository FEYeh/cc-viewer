...
Вызов Task порождает цепочку запросов SubAgent, видимую в списке запросов как независимая последовательность подзапросов, отделённая от MainAgent. Запросы SubAgent обычно имеют упрощённый system prompt и меньше определений инструментов, что явно отличает их от MainAgent.
