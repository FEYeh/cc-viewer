...
Виклики інструментів є одними з найважливіших спостережуваних даних у журналах запитів Claude Code. Масив `tools` кожного запиту MainAgent визначає набір інструментів, доступних моделі, а `tool_use` / `tool_result` content block у відповіді фіксують фактичні виклики інструментів та їх результати. cc-viewer аналізує ці дані для відновлення повного ланцюга викликів інструментів у Chat Mode.

Зміни у визначеннях інструментів (наприклад, додавання/видалення інструментів через підключення/відключення MCP server) спричиняють перебудову кешу, детальніше див. [CacheRebuild](CacheRebuild.md).
