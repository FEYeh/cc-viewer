...
- Блоки thinking та повідомлення Assistant підтримують переклад одним натисканням
- На основі API Claude Haiku, підтримує автентифікацію як за API Key (`x-api-key`), так і за OAuth Bearer Token
- Результати перекладу автоматично кешуються; натисніть знову, щоб переключитися на оригінальний текст
- Під час перекладу відображається анімація завантаження
...
- Експорт промптів користувача: витягнення та відображення всіх введень користувача, XML-теги (system-reminder тощо) згортаються; слеш-команди (`/model`, `/context` тощо) відображаються як окремі записи; теги, пов'язані з командами, автоматично приховуються зі вмісту промпту
