...
Ця команда автоматично впроваджує скрипт моніторингу у ваш локально встановлений Claude Code та додає hook автоматичного повторного впровадження до конфігурації вашого shell (`~/.zshrc` або `~/.bashrc`). Потім використовуйте Claude Code як зазвичай та відкрийте `http://localhost:7008` у браузері для перегляду інтерфейсу моніторингу.

Після оновлення Claude Code ручні дії не потрібні — наступного разу, коли ви запустите `claude`, він автоматично виявить та повторно впровадить.
...
Очищує впровадження cli.js та hook конфігурації shell за один крок.
...
- Повідомлення користувача вирівняні ліворуч (сині бульбашки)
- Відповіді Main Agent вирівняні ліворуч (темно-сірі бульбашки) з рендерингом Markdown
...
- Теги системного впровадження (`<system-reminder>`, `<project-reminder>` тощо) автоматично згорнуті
- Текст, впроваджений системою, автоматично фільтрується, показуючи лише реальне введення користувача
...
- Збереження поточного логу: завантаження поточного файлу логу моніторингу JSONL
