...
### Как отправить PR
Автор рад принимать PR и поощряет сообщество к их отправке.

Несколько требований:

При отправке PR, пожалуйста, сообщите мне, какой был ваш Prompt и какую модель вы использовали для изменения кода (PR, сделанные с помощью слабых моделей, не принимаются);
Если есть изменения интерфейса, сообщите мне, какая функциональность была изменена — рекомендуется скриншот с обведёнными изменениями;
Изменения в cli.js и interceptor.js будут проверяться очень тщательно, так как я не хочу, чтобы проблемы в основных файлах повлияли на использование всеми;
Пожалуйста, обязательно проверьте функциональность локально перед отправкой — огромное спасибо!
