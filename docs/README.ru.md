...
- Skills reminder автоматически обнаруживаются и сворачиваются
...
- Блоки thinking и сообщения Assistant поддерживают перевод в один клик
- На основе Claude Haiku API, используется только аутентификация `x-api-key` (токены сессии OAuth исключены для предотвращения загрязнения контекста)
- Автоматически захватывает имя модели haiku из запросов mainAgent; по умолчанию `claude-haiku-4-5-20251001`
- Результаты перевода автоматически кэшируются; нажмите снова для переключения на оригинальный текст
- Анимация загрузки во время перевода
- Иконка (?) рядом с заголовком `authorization` в деталях запроса ведёт к концептуальному документу о загрязнении контекста
...
- Статистика использования Skill: частота вызовов по каждому Skill, отсортированная по частоте
