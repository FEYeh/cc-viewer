...
- Список запросов автоматически прокручивается к выбранному элементу (по центру при переключении режима, к ближайшему при ручном клике)
...
- Раздел Diff поддерживает переключение между представлениями JSON/Text и копирование одним нажатием
- Настройка «Expand Diff»: при включении запросы MainAgent автоматически раскрывают раздел Diff
- Подсказка Body Diff JSON может быть закрыта; после закрытия настройка сохраняется на сервере и больше не отображается
- Чувствительные заголовки (`x-api-key`, `authorization`) автоматически маскируются в файлах логов JSONL для предотвращения утечки учётных данных
...
- Совместимость с Claude Code Router (CCR) и другими прокси-конфигурациями — запросы определяются по шаблону пути API в качестве резервного метода
...
- Блоки `thinking` свёрнуты по умолчанию, отображаются в формате Markdown, нажмите для раскрытия; поддержка перевода одним нажатием
...
- Результаты инструментов Task (SubAgent) отображаются в формате Markdown
...
- Сообщения о загрузке Skill автоматически обнаруживаются и сворачиваются, отображая имя Skill; нажмите для раскрытия полной документации (рендеринг Markdown)
...
- Каждое сообщение показывает временную метку с точностью до секунды, рассчитанную на основе времени API-запроса
- Каждое сообщение содержит ссылку «Просмотр запроса» для перехода в режим raw к соответствующему API-запросу
- Двунаправленная синхронизация режимов: при переключении в режим чата прокручивается к диалогу, соответствующему выбранному запросу; при переключении обратно прокручивается к выбранному запросу
- Панель настроек: переключение состояния свёртывания по умолчанию для результатов инструментов и блоков размышлений
- Глобальные настройки: переключение фильтрации нерелевантных запросов (count_tokens, heartbeat)

### Перевод

- Блоки thinking и сообщения Assistant поддерживают перевод одним нажатием
- На основе API Claude Haiku, поддерживает аутентификацию как по API Key (`x-api-key`), так и по OAuth Bearer Token
- Результаты перевода автоматически кэшируются; нажмите снова, чтобы переключиться на оригинальный текст
- Во время перевода отображается анимация загрузки
...
- Статистика пересоздания кэша, сгруппированная по причине (TTL, изменение system/tools/model, усечение/изменение сообщений, изменение ключа) с количеством и токенами cache_creation
- Статистика использования инструментов: количество вызовов по каждому инструменту, отсортированное по частоте
- Иконки концептуальной справки (?): нажмите для просмотра встроенной документации MainAgent, CacheRebuild и каждого инструмента
...
- Экспорт пользовательских промптов: извлечение и отображение всех пользовательских вводов, XML-теги (system-reminder и др.) сворачиваемые; слеш-команды (`/model`, `/context` и др.) отображаются как отдельные записи; теги, связанные с командами, автоматически скрываются из содержимого промпта
- Экспорт промптов в TXT: экспорт пользовательских промптов (только текст, без системных тегов) в локальный файл `.txt`
