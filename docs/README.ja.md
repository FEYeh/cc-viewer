...
### PRの提出方法について
作者はPRを歓迎しており、コミュニティからのPR提出を積極的に奨励しています。

いくつかの要件：

PRを提出する際は、使用したPromptと、コード修正に使用したモデルを教えてください（品質の低いモデルで修正されたコードは受け付けません）；
UI変更がある場合は、インターフェース上で修正した機能を教えてください — スクリーンショットに丸を描いて示すことを推奨します；
cli.jsとinterceptor.jsの変更は非常に慎重にレビューします。コアファイルに問題が発生して皆さんの利用に影響を与えたくありません；
必ずローカルで機能を検証してから提出してください。よろしくお願いいたします！
