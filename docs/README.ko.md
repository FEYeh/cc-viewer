...
이 명령은 로컬에 설치된 Claude Code에 모니터링 스크립트를 자동으로 주입하고, shell 설정 파일(`~/.zshrc` 또는 `~/.bashrc`)에 자동 재주입 hook을 추가합니다. 이후 Claude Code를 정상적으로 사용하고 브라우저에서 `http://localhost:7008`을 열어 모니터링 인터페이스를 확인하세요.

Claude Code 업데이트 후 수동 작업이 필요 없습니다. 다음에 `claude`를 실행하면 자동으로 감지하고 재주입합니다.
...
cli.js 주입 코드와 shell 설정 파일의 hook을 한 번에 정리합니다.
...
- 사용자 메시지 왼쪽 정렬 (파란색 말풍선)
- Main Agent 응답 왼쪽 정렬 (진한 회색 말풍선), Markdown 렌더링 지원
...
- 시스템 주입 태그 (`<system-reminder>`, `<project-reminder>` 등) 자동 접힘
- 시스템 주입 텍스트 자동 필터링, 실제 사용자 입력만 표시
...
- 현재 로그 저장: 현재 모니터링 중인 JSONL 로그 파일 다운로드
