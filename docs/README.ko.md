...
### PR 제출 방법
작성자는 PR을 환영하며, 커뮤니티의 PR 제출을 적극 권장합니다.

몇 가지 요구사항:

PR을 제출할 때, 사용한 Prompt가 무엇인지, 그리고 코드 수정에 어떤 모델을 사용했는지 알려주세요 (품질이 낮은 모델로 수정한 코드는 받지 않습니다);
UI 변경이 있는 경우, 인터페이스에서 수정된 기능이 무엇인지 알려주세요 — 스크린샷에 동그라미를 그려서 표시하는 것을 추천합니다;
cli.js와 interceptor.js의 변경은 매우 신중하게 검토합니다. 핵심 파일에 문제가 생겨 모든 사용자에게 영향을 주는 것을 원하지 않습니다;
반드시 로컬에서 기능을 검증한 후 제출해 주세요. 대단히 감사합니다!
